...
public class AlexLionTest {
    ...
    @Mock
     Feline feline; // Мок для Predator (создаем мок-объект интерфейса Predator)
    ...
    public void setUp() throws Exception {
        alexLion = new AlexLion(feline); // Создаем AlexLion с моком Predator (инициализируем объект AlexLion, передавая ему мок-объект predator)
    }

    @Test // Аннотация, указывающая, что метод getFriends_ReturnsCorrectListOfFriends() является тестовым методом
    public void getFriendsReturnsCorrectListOfFriends() { // Проверяем, что getFriends() возвращает правильный список друзей (тест проверяет, что метод getFriends() класса AlexLion возвращает ожидаемый список друзей)
    ...
    public void getPlaceOfLivingReturnsNewYorkZoo() { // Проверяем, что getPlaceOfLiving() возвращает "New York Zoo" (тест проверяет, что метод getPlaceOfLiving() класса AlexLion возвращает строку "New York Zoo")
    ...
    public void getKittensReturnsZero() { // Проверяем, что getKittens() возвращает 0 (тест проверяет, что метод getKittens() класса AlexLion всегда возвращает 0, так как у Алекса нет котят)
    ...
    public void doesHaveManeReturnsTrue() { // Проверяем, что doesHaveMane() возвращает true (т.к. Алекс - самец) (тест проверяет, что метод doesHaveMane() класса AlexLion возвращает true, так как Алекс всегда самец и, следовательно, имеет гриву)
